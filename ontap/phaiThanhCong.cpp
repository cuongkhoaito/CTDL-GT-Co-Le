...
int demChanChia5(int a[100][100], int n){
	int d=0;
	for(int i=1; i<=n; i++){		
		for(int j=1; j<=n; j++){
...
int demC5Break(int a[100][100], int n){
	int d=0;
	for(int i=1; i<=n; i++){		
		for(int j=1; j<=n; j++){
			if(kiemTraChanChia5(a[i][j])==1){
...
int kiemTraChen(int a[100][100], int n, int cot){		 	// cot thay cho i
	for(int hang=1; hang<=n; hang++){ 						// hang thay cho j
		if(a[cot][hang] %2==0 && a[cot][hang]%5==0){
...
main(){
	...
	printf("\n\n");
	in(maTran,n);	
	printf(" \n cau 3 xuat chan chia 5 la: %d   ",demChanChia5(maTran, n));
	nm=demC5Break(maTran, n);
	ds = new int[ nm+ 1];
